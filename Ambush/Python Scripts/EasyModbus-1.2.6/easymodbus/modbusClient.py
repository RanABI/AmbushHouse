...
class ModbusClient(object):
    ...
    def read_discreteinputs(self, starting_address, quantity):
        ...
        if (self.__ser is not None):
            ...
            if (len(data) < bytes_to_read):
                raise TimeoutError('Read timeout Exception')
            if ((data[1] == 0x82) & (data[2] == 0x01)):
                raise FunctionCodeNotSupportedException("Function code not supported by master");
            if ((data[1] == 0x82) & (data[2] == 0x02)):
                raise StartingAddressInvalidException("Starting address invalid or starting address + quantity invalid");
            if ((data[1] == 0x82) & (data[2] == 0x03)):
                raise QuantityInvalidException("quantity invalid");
            if ((data[1] == 0x82) & (data[2] == 0x04)):
                raise ModbusException("error reading");
            crc = self.__calculateCRC(data, len(data) - 2, 0)
            crcLSB = crc&0xFF
            crcMSB = (crc&0xFF00) >> 8
            if ((crcLSB != data[len(data)-2]) & (crcMSB != data[len(data)-1])):
                raise CRCCheckFailedException("CRC check failed");
            myList = list()
    ...
    def read_inputregisters(self, starting_address, quantity):
        ...
        if (self.__ser is not None):
            ...
            if (len(data) < bytes_to_read):
                raise TimeoutError("Read timeout Exception")
            if ((data[1] == 0x84) and (data[2] == 0x01)):
                raise FunctionCodeNotSupportedException("Function code not supported by master");
            if ((data[1] == 0x84) and (data[2] == 0x02)):
                raise StartingAddressInvalidException("Starting address invalid or starting address + quantity invalid");
            if ((data[1] == 0x84) and (data[2] == 0x03)):
                raise QuantityInvalidException("quantity invalid");
            if ((data[1] == 0x84) and (data[2] == 0x04)):
                raise ModbusException("error reading");
            crc = self.__calculateCRC(data, len(data) - 2, 0)
            crcLSB = crc&0xFF
            crcMSB = (crc&0xFF00) >> 8
            ...
            try:
                ...
                raise Exception('Read Timeout')
            data = bytearray(self.__receivedata)
            if ((data[1+6] == 0x84) and (data[2+6] == 0x01)):
               raise FunctionCodeNotSupportedException("Function code not supported by master");
            if ((data[1+6] == 0x84) and(data[2+6] == 0x02)):
                raise StartingAddressInvalidException("Starting address invalid or starting address + quantity invalid");
            if ((data[1+6] == 0x84) and(data[2+6] == 0x03)):
                raise QuantityInvalidException("quantity invalid");
            if ((data[1+6] == 0x84) and (data[2+6] == 0x04)):
                raise ModbusException("error reading");
            myList = list()
            for i in range(0, quantity):
                myList.append((data[i*2+3+6]<<8) +data[i*2+4+6])            
    ...
    def write_single_coil(self, starting_address, value):
        ...
        if (self.__ser is not None):
            ...
            if ((data[1] == 0x85) & (data[2] == 0x02)):
                raise StartingAddressInvalidException("Address invalid");
            if ((data[1] == 0x85) & (data[2] == 0x03)):
                raise QuantityInvalidException("Value invalid");
    ...
    def write_single_register(self, starting_address, value):
        ...
        if (self.__ser is not None):
            ...
            if ((data[1] == 0x86) & (data[2] == 0x02)):
                raise StartingAddressInvalidException("Register address invalid");
            if ((data[1] == 0x86) & (data[2] == 0x03)):
                raise QuantityInvalidException("Invalid Register Value");
